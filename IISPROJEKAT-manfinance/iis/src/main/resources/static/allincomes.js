$(document).ready(function (){
    if(localStorage.getItem('role') === "MANFINANCE"){
        ...
        let url = new URL('http://localhost:8011/api/income/all');
        ...
        $.ajax({
            ...
            success: function (res) {
               ...
               // Function to filter table rows based on search input and category
                ...
                $("#allincomes th:nth-child(5)").click(function() {
                ...
                $("#allincomes th:nth-child(1)").click(function() {
                    var table = $(this).closest("table");
                ...
                for ( let i = 0; i < res.length; i++) {
                    ...
                    row += "<td>" + res[i].incomeSource + "</td>";
                    row += "<td>" + res[i].description + "</td>";

                    if (res[i].incomeCategory === "MEMBERSHIPFEE") {
                        res[i].incomeCategory = "Članarina";
                    } else if (res[i].incomeCategory === "RACECOMPETITIONFEE") {
                        res[i].incomeCategory = "Kotizacija za trku ili takmičenje";
                    } else if (res[i].incomeCategory === "EDPROGRAMFEE") {
                        res[i].incomeCategory = "Uplata za obrazovni program";
                    } else {
                        res[i].incomeCategory = "Ostalo";
                    }

                    row += "<td>" + res[i].incomeCategory + "</td>";
                    row += "<td>" + res[i].amount + "</td>";
                    row += "</tr>";
                    $('#allincomes').append(row);
                }
            },
            error: function () {
                alert("Greska!");
